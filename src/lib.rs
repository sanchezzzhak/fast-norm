...
fn core_denormalize(src: &[f32], dst: &mut [u8], size: usize, mean: &[f32], std_val: &[f32], is_div_255: bool) -> Result<()> {
  if src.len() < size * 3 || dst.len() < size * 3 || mean.len() < 3 || std_val.len() < 3 {
    return Err(Error::new(Status::InvalidArg, "Invalid buffer sizes".to_string()));
  }
...
impl Task for DenormalizeTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> { core_denormalize(&self.src_buf, &mut self.dst_buf, (self.width * self.height) as usize, &self.mean, &self.std_val, self.is_div_255) }
